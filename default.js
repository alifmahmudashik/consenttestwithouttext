function pageview(){
    ...
    gtag('consent', 'update', defaultConsentPageView[0]);
}

pageview();

function consentUpdate(){
    var getCookieForConsent = JSON.parse(localStorage.getItem("consent"));
    // Define dataLayer and the gtag function.
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}

    gtag('consent', 'update', getCookieForConsent[0]);

    window.dataLayer.push({
        event: "consent_update",
        consent: getCookieForConsent[0]
    })
